...
c_high = np.array([0.07, 0.6, 0.07, 0.05, 0.07, 0.065,0.07,0.07,0.07]) * 2.5
c_low = [-c_high[0], -c_high[1], -c_high[2], -c_high[3], -c_high[4], -c_high[5],-c_high[6],-c_high[7],-c_high[8]]
...
def move_to_position(sim,controller,target,tol = 0.05,render=False,viewer=None):
...
class PIDcontroller(object):
    def __init__(self,KP,KI,KD,c_high,c_low = 0):
        self.KP = KP
        self.KI = KI
        self.KD = KD
        self.c_high = c_high
    ...
    def act(self,current,target):
        ...
        if len(list(self.accum_error)) > 0:
            action = action + self.KD * (error - self.accum_error[-1])
...
def default_controller_init():
    ...
    KP = np.array([0.5, 2.5, 0.9, 0.9, 0.9, 0.,0.,0.,0.]) *7
    KI = np.array([0.001, 0.001, 0.001, 0.005, 0.005, 0.,0.,0.,0.])*0.1
    KD = KP * 4

    controller = PIDcontroller(KP, KI, KD, c_high, c_low)

    return controller

def default_controller_init_PI():
